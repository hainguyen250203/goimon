...
export function PromotionRowActions({
  ...
  const remove = api.promotion.delete.useMutation({
    onSuccess: () => {
      toaster.create({ title: "Đã xoá khuyến mãi", type: "success" });
  ...
  return (
    <>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Xoá khuyến mãi &quot;{item.name}&quot;?</DialogTitle>
          </DialogHeader>
          <DialogBody>
            Hành động này không thể hoàn tác. Các đơn hàng đã áp dụng khuyến mãi này vẫn giữ
            nguyên số tiền đã giảm.
          </DialogBody>
          <DialogFooter>
